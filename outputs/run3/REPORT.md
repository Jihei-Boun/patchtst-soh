...
`run2_fast`와 **동일한 기본 하이퍼파라미터**로 `outputs/run3`에 재실행하여, 결과 재현성 및 산출물 정리를 확인한다.
...
```text
checkpoints/run3/best_model.pt
```
...
- 예측 대상: `soh`
- 분할: 차량별 시간순 70% / 15% / 15%
- 다운샘플 (`sample_stride=10`) 후 행 수: 135,020 / 228,909 / 209,842 / 21,051
...
동일 설정(차량 4대, sample_stride=10, window_stride=8, epochs=8)으로 재실행한 결과이며, 지표는 `run2_fast`와 동일 수준이다.

| 실험 | Test MAE | Test RMSE |
|---|---:|---:|
| run2_fast | 1.036 | 1.236 |
| run3 | 1.036 | 1.236 |
...
| 파일 | 설명 |
|---|---|
| `outputs/run3/metrics.json` | 전체/차량별 지표 및 설정값 |
| `outputs/run3/prediction.csv` | 실제 SOH / 예측 SOH / 절대오차 |
| `outputs/run3/prediction.png` | 실제 vs 예측 SOH 그래프 |
| `outputs/run3/training_loss.png` | Train / Validation loss 곡선 |
| `checkpoints/run3/best_model.pt` | 최고 성능 모델 체크포인트 |
...
4. 동일 seed·동일 설정이면 `run2_fast`와 거의 같은 MAE/RMSE가 재현된다.
5. 차량별로 성능 차이가 크며, `01241225211`이 가장 낮고 `01241225220`이 상대적으로 높다.
6. `best_model.pt`는 바이너리이므로 에디터로 열지 않고, 재추론/재사용용으로 보관한다.
...
- PatchTST 기반 SOH 예측을 **차량 4대**로 학습·평가하고, 결과를 `outputs/run3/`에 저장했다.
- 최종 성능은 **Test MAE 1.036 / Test RMSE 1.236**이다.
- `run2_fast`와 동일 설정 재실행으로, 파이프라인 재현성을 확인했다.
- 코드·결과·체크포인트는 `outputs/run3/` 및 `checkpoints/run3/`에 정리되어 있다.
